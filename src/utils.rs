...
pub fn parse_FEN_row(
    row: &str,
    mut piece_index: usize,
    mut onebit_index: usize,
...
pub fn parse_algebraic_move(move_input: &str, game: &mut Game) -> Option<Move> {
    ...
    for possible_move in possible_moves {
        if move_to_ambiguous_algebraic_notation(game, possible_move)
            == Some(move_input.to_owned().to_ascii_lowercase())
        {
            possible_matches.push(possible_move);
        }
        if move_to_unambiguous_algebraic_notation(game, possible_move)
            == Some(move_input.to_owned().to_ascii_lowercase())
        {
            possible_matches.push(possible_move);
        }
    ...
    if possible_matches.len() == 1 {
        ...
        if possible_matches
            .iter()
            .all(|m| m.from_square == possible_matches.iter().next().unwrap().from_square)
        {
            let mut promotion_piece_type: Option<PieceType> = None;
            while promotion_piece_type == None {
                print_board(&game);
                print!(
                    "Piece to promote to (Q for Queen, R for Rook, N for Knight, B for Bishop): "
                );
                io::stdout().flush().unwrap();
                ...
                if promotion_input != "" {
                    promotion_piece_type =
                        match promotion_input.chars().next().unwrap().to_ascii_lowercase() {
            ...
            if let Some(matched) = possible_matches
                .iter()
                .find(|&m| m.promotion == promotion_piece_type)
            {
                return Some(*matched);
            }
        } else {
...
pub fn move_to_unambiguous_algebraic_notation(game: &Game, possible_move: Move) -> Option<String> {
    ...
    if let Some(piece) = game
        .pieces
        .iter()
        .find(|p| p.taken == false && p.bit == from_bit && p.colour == game.active_colour)
    {
...
pub fn uci_notation_to_move(uci_notation_move: String) -> Move {
    let from = &uci_notation_move[0..2];
    let to = &uci_notation_move[2..4];
    let from_square = coords_to_onebit_index(from);
    let to_square = coords_to_onebit_index(to);
    Move {
        from_square: from_square.expect("Invalid move"),
        to_square: to_square.expect("Invalid move"),
        promotion: None,
        capture_square: None,
    }
}
